...
export class AddStaffSubstitudeComponent implements OnInit {
  ...
  displayedColumns: string[] = ['date', 'class', 'subject', 'period', 'subtitute', 'Action'];
  dataSource: any;
  ...
  constructor(private router: Router, public dialog: MatDialog, private fb: FormBuilder,
    private service: CommonService, private cdr: ChangeDetectorRef,
  ...
  getStaffList() {
    ...
    this.service.getHttpServiceWithDynamicParams(postData, 'substituteStaffDropDown').subscribe((response: any) => {
      if (response.status) {
  ...
  get filteredStaffList() {
    ...
    return this.BindStaffList.filter((element: any) => element?.staff?.toLowerCase().includes(lowerCaseSearch));
  }
  ...
  getStaffSubstituteGrid() {
    ...
    let postData = {
      schoolcode: localStorage.getItem('schoolcode'),
      datefrom: this.form.value.fromDate ? this.datePipe.transform(this.form.value.fromDate, 'dd/MM/yyyy') : '',
      dateto: this.form.value.toDate ? this.datePipe.transform(this.form.value.toDate, 'dd/MM/yyyy') : '',
      staffcode: this.form.value.staff
    }
    this.service.getHttpServiceWithDynamicParams(postData, 'getStaffSubstituteGrid').subscribe((response: any) => {
      if (response.status) {
